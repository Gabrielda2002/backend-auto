...
import { useResumen } from '@/lib/queries';
            ...
            <BlurFade delay={0.12}>
              <section className="grid grid-cols-12 gap-6">
                ...
                <CardHeader className="flex flex-row items-start justify-between p-0 pb-6">
                  <div>
                    <CardTitle>Evolución Mensual de Citas vs Cumplidas</CardTitle>
                    <p className="text-label-md text-on-surface-variant">Consolidado por mes</p>
                  </div>
                  <div className="flex gap-4 text-[12px] font-bold text-on-surface-variant">
                    <span className="flex items-center gap-1">
                      <span className="h-3 w-3 rounded-full bg-corporate-navy" /> Total
                    </span>
                    <span className="flex items-center gap-1">
            ...
            <section className="grid grid-cols-12 gap-6">
              ...
              <div className="col-span-12 lg:col-span-6">
                <Carousel
                  ...
                  description={`${formatNumber(data.meta?.total ?? 0)} citas · ${data.volumenPorSede.length} sedes`}
...
function cumplAccent(pct: number | null | undefined): 'green' | 'turquoise' | 'amber' | 'red' {
  if (pct == null) return 'turquoise';
  if (pct >= 80) return 'green';
  if (pct >= 50) return 'turquoise';
  if (pct >= 30) return 'amber';
  return 'red';
}

function pctTone(pct: number | null | undefined): 'green' | 'turquoise' | 'amber' | 'red' {
